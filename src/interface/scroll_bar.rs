...
impl ScrollBar {
	pub fn new(engine: Arc<Engine>, style: Option<ScrollBarStyle>, parent: Option<Arc<Bin>>, scroll: Arc<Bin>) -> Arc<Self> {
		...
		sb.back.add_hook_raw(BinHook::MouseScroll, Arc::new(move |_, data| {
			if let BinHookData::MouseScroll { scroll_amt, .. } = data {
				match sb_wk.upgrade() {
		...
		sb.scroll.add_hook_raw(BinHook::MouseScroll, Arc::new(move |_, data| {
			if let BinHookData::MouseScroll { scroll_amt, .. } = data {
				match sb_wk.upgrade() {
					Some(sb) => sb.update(ScrollTo::Amount(*scroll_amt)),
					None => ()
