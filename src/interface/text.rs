...
impl Text {
    ...
    pub(crate) fn render_text<T: Into<String>, F: Into<String>>(
        ...
        wrap: WrapTy,
        align: TextAlign,
        line_height_op: Option<f32>,
        line_limit_op: Option<usize>,
        ...
        unsafe {
            ...
            let (_, ft_face_ap) = {
                ...
                if ft_faces.contains_key(&size) {
                    ...
                    let ret = (
                        Arc::new(AtomicPtr::new(ft_library)),
                        Arc::new(AtomicPtr::new(ft_face)),
                    );
            ...
            let size_info = {
                ...
                if size_infos.contains_key(&size) {
                    ...
                    let info = ::std::slice::from_raw_parts(
                        hb_buffer_get_glyph_infos(hb_buffer, ptr::null_mut()),
                        len,
                    );
                    let pos = std::slice::from_raw_parts(
                        hb_buffer_get_glyph_positions(hb_buffer, ptr::null_mut()),
                        len,
                    );

                    let t_metrics = match FT_Load_Glyph(
                        ft_face,
                        info[0].codepoint.into(),
                    ...
                    let g_metrics = match FT_Load_Glyph(
                        ft_face,
                        info[1].codepoint.into(),
                        FT_LOAD_DEFAULT as i32,
                    ) {
                        0 => (*(*ft_face).glyph).metrics.clone(),
                        e => return Err(format!("FT_Load_Glyph: error {}", e)),
                    ...
                    let top =
                        (pos[0].y_offset as f32 / 64.0) - (t_metrics.horiBearingY as f32 / 64.0);
                    let bottom = (pos[1].y_offset as f32 - g_metrics.horiBearingY as f32
                        + g_metrics.height as f32)
                        / 64.0;
            ...
            for ctext in clines {
                ...
                for i in 0..len {
                    let glyph_info: Arc<Glyph> = {
                        ...
                        if glyphs.contains_key(&size)
                            && glyphs
                                .get(&size)
                                .unwrap()
                                .contains_key(&(info[i].codepoint as u64))
                        {
                            glyphs
                                .get(&size)
                                .unwrap()
                                .get(&(info[i].codepoint as u64))
                                .unwrap()
                                .clone()
                        } else {
                    ...
                    if glyph_info.w == 0.0 || glyph_info.h == 0.0 {
                        ...
                        lines
                            ...
                            .last_mut()
                            .unwrap()
                            .push((glyph_info.coords.img_id, verts));
                    }
