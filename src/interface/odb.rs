...
struct Buffer {
    ...
    draw_sets: Vec<(
        BufferSlice<[ItfVertInfo], Arc<DeviceLocalBuffer<[ItfVertInfo]>>>,
        atlas::AtlasImageID,
        Option<Arc<ImageViewAccess + Send + Sync>>,
    )>,
    resize: bool,
    win_size: [f32; 2],
    scale: f32,
...
impl Buffer {
    ...
    fn update(&mut self) -> bool {
        ...
        let bins: Vec<_> = self
            ...
            .read()
            .iter()
            .filter_map(|(id, w)| w.upgrade().map(|v| (*id, v)))
            .collect();
        ...
        ::misc::do_work(
            bins.iter()
                .filter_map(|(_, b)| match b.wants_update() || self.resize {
                    true => Some(b.clone()),
                    false => None,
                })
                .collect(),
        ...
        for (id, bin) in bins {
            ...
            let mut prexisting_chunks: Vec<_> =
                self.chunks.iter().filter(|c| c.bin_id == id).collect();
            ...
            up_bin_ids.push(id);
            let mut data_mapped: BTreeMap<R32, BTreeMap<u64, Vec<(Vec<ItfVertInfo>, Option<_>)>>> =
                BTreeMap::new();
            ...
            for (data, image_op, mut atlas_id) in bin.verts_cp() {
                ...
                for vert in data.into_iter() {
                    ...
                    let mut entry_point = data_mapped
                        .entry(R32::from(-1.0 * tri[0].position.2))
                    ...
                    if entry_point.is_empty() {
                        ...
                        for (d, iop) in &mut *entry_point {
                            if (iop.is_none() && image_op.is_none())
                                || (iop.is_some()
                                    && image_op.is_some()
                                    && Arc::ptr_eq(
                                        iop.as_ref().unwrap(),
                                        image_op.as_ref().unwrap(),
                                    ))
                            {
                                d.append(&mut tri);
                                found = true;
        ...
        for i in (0..self.chunks.len()).rev() {
            if up_bin_ids.contains(&self.chunks[i].bin_id)
        ...
        if !self.chunks.is_empty() {
            ...
            for c_i in 1..self.chunks.len() {
                ...
                if self.chunks[c_i].atlas_id != last_atlas
                    ...
                    || c_i == self.chunks.len() - 1
                {
                    ...
                    self.draw_sets.push((
                        dst_buf
                            ...
                            .slice(start..cur_pos)
                            .unwrap(),
                        self.chunks[c_i - 1].atlas_id,
                        self.chunks[c_i - 1].image_op.clone(),
                    ));
