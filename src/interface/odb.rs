...
impl OrderedBuffer {
	...
	fn update_draw_data(&mut self, force_up: bool) {
		if let Some((version, image_views)) = self.basalt.atlas_ref().image_views() {
			if self.draw_version.is_some() && *self.draw_version.as_ref().expect("1") == version && !force_up {
				return;
			}
			...
			for (buf, atlas_id, image_op) in &self.draw {
				let image: Arc<dyn ImageViewAccess + Send + Sync> = match atlas_id {
					...
					}, img_id => match image_views.get(img_id) {
	...
	fn update(&mut self, force_all: bool) -> bool {
		...
		for (bin_id, bin) in &alive_bins {
			if !new_bin_ids.contains(bin_id) {
				if force_all || match self.contains.get(bin_id) {
		...
		for (si, di, len) in copy_regions {
			cmdbuf = cmdbuf.copy_buffer(
				old_buf.as_ref().expect("6").clone().into_buffer_slice().slice(si..(si+len)).expect("7"),
		...
		drop(cmdbuf
			.build().expect("13")
			.execute(self.basalt.transfer_queue()).expect("14")
			.then_signal_semaphore_and_flush().expect("15")
			.cleanup_finished());
		...
		for (_, image_mapped) in sorted.iter_mut().rev() {
			for (_, chunks) in &mut *image_mapped {
				...
				for (start, len) in ranges {
					...
					draw.push((
						new_buf.clone().into_buffer_slice().slice(start..(start+len)).expect("19"),
						atlas_id, image_op
					));
				}
			}
