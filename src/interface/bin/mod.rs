...
pub use self::style::BinPosition;
pub use self::style::Color;
...
impl Bin {
	...
	fn pos_size_tlwh(&self, win_size_: Option<[f32; 2]>) -> (f32, f32, f32, f32) {
		...
		let (par_t, par_b, par_l, par_r) = match style.position.clone().unwrap_or(BinPosition::Window) {
			BinPosition::Window => (0.0, win_size[1], 0.0, win_size[0]),
			BinPosition::Parent => match self.parent() {
				Some(ref parent) => {
