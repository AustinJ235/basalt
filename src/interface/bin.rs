...
pub struct Bin {
    ...
    verts: Mutex<
        Vec<(
            Vec<ItfVertInfo>,
            Option<Arc<vulkano::image::traits::ImageViewAccess + Send + Sync>>,
            u64,
        )>,
    >,
    id: u64,
...
impl Bin {
    ...
    pub fn take_children(&self) -> Vec<Arc<Bin>> {
        self.children
            ...
            .split_off(0)
            .into_iter()
            .filter_map(|child_wk| match child_wk.upgrade() {
                Some(child) => {
    ...
    pub fn add_drag_events(self: &Arc<Self>, target_op: Option<Arc<Bin>>) {
        ...
        let target_wk = target_op
            ...
            .unwrap_or(Arc::downgrade(self));
        let data_cp = data.clone();

        self.input_hook_ids
            .lock()
            .push(self.basalt.input_ref().on_mouse_press(
                MouseButton::Middle,
                Arc::new(move |data| {
        ...
        self.input_hook_ids
            .lock()
            .push(self.basalt.input_ref().add_hook(
                InputHook::MouseMove,
                Arc::new(move |data| {
        ...
        self.input_hook_ids
            .lock()
            .push(self.basalt.input_ref().on_mouse_release(
                MouseButton::Middle,
                Arc::new(move |_| {
                    *data_cp.lock() = None;
                    InputHookRes::Success
    ...
    pub fn add_enter_text_events(self: &Arc<Self>) {
        self.add_hook_raw(
            ...
            Arc::new(move |bin, data| {
                if let BinHookData::Character { char_ty, .. } = data {
        ...
        let from_t = match pos_from_t {
            ...
            None => match pos_from_b {
                Some(from_b) => match style.height {
                    ...
                    None => {
                        println!(
                            "UI Bin Warning! ID: {}, Unable to get position \
                             from top, position from bottom is specified \
                             but no height was provied.",
                            self.id
                        );
                        0.0
                    }
                ...
                None => {
                    println!(
                        "UI Bin Warning! ID: {}, Unable to get position \
                         from top, position from bottom is non specified.",
                        self.id
                    );
                    0.0
                }
                ...
                None => {
                    println!(
                        "UI Bin Warning! ID: {}, Unable to get position from\
                         left, position from right is not specified.",
                        self.id
                    );
                    0.0
                }
    ...
    pub(crate) fn do_update(self: &Arc<Self>, win_size: [f32; 2], scale: f32) {
        ...
        let ancestor_data: Vec<(Arc<Bin>, BinStyle, f32, f32, f32, f32)> = self
            .ancestors()
            .into_iter()
            .map(|bin| {
                let (top, left, width, height) = bin.pos_size_tlwh(Some(scaled_win_size));
                (bin.clone(), bin.style_copy(), top, left, width, height)
            })
            .collect();
        ...
        let mut text_color = style.text_color.unwrap_or(Color {
            r: 0.0,
            g: 0.0,
            b: 0.0,
            a: 1.0,
            ...
            if back_color.a > 0.0 || back_coords.img_id != 0 || back_img.is_some() {
                ...
                if border_radius_tr != 0.0 {
                    ...
                    for i in 0..triangles {
                        ...
                        back_verts.push(points[i + 1].clone());
                        back_verts
                            .push((bps.tri[0] - border_radius_tl, bps.tri[1] + border_radius_tr));
                    }
                }
        ...
        match self.basalt.interface_ref().text_ref().render_text(
            ...
            wrap_ty,
            text_align,
            style.line_height.map(|v| v * scale),
            style.line_limit,
        ) {
            Ok(text_verts) => {
