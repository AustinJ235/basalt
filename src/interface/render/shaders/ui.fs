...
vec4 textureBicubic(vec2 texCoords) {
    vec2 texSize = textureSize(nonuniformEXT(images[tex_i]), 0);
    ...
    vec4 sample0 = textureLod(nonuniformEXT(images[tex_i]), offset.xz, 0);
    vec4 sample1 = textureLod(nonuniformEXT(images[tex_i]), offset.yz, 0);
    vec4 sample2 = textureLod(nonuniformEXT(images[tex_i]), offset.xw, 0);
    vec4 sample3 = textureLod(nonuniformEXT(images[tex_i]), offset.yw, 0);
...
void main() {
    if(type == -1) { // Clear
        ...
        out_color = vec4(
            color.rgb,
            textureLod(nonuniformEXT(images[tex_i]), coords, 0).r
        );
    }
