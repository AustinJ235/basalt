...
impl BinHook {
    fn into_data(self) -> BinHookData {
        match self {
            ...
            BinHook::MouseScroll => BinHookData::MouseScroll { scroll_amt: 0.0 },
...
impl HookManager {
    ...
    pub fn add_hook(&self, bin: Arc<Bin>, hook: BinHook, func: BinHookFn) -> BinHookID {
        ...
        *current_id += 1;
        drop(current_id);
        self.add
            .send((id, (Arc::downgrade(&bin), hook.into_data(), func)))
            .unwrap();
        id
    }
                                ...
                                for (hook_id, (hb_wk, hook, func)) in &mut *hooks {
                                    ...
                                    if bin.id() == hb.id() {
                                        if hook.ty() == BinHookTy::MouseEnter {
                                            if let BinHookData::MouseEnter { mouse_x, mouse_y } =
                                                hook
                                            {
                                                *mouse_x = m_window_x;
                                                *mouse_y = m_window_y;
                ...
                if SMOOTH_SCROLL {
                    if m_scroll_amt != 0.0 {
                        if SMOOTH_SCROLL_ACCEL {
                            smooth_scroll.to += m_scroll_amt
                                * SMOOTH_SROLLL_STEP_MULT
                                * ((smooth_scroll.to).abs() + SMOOTH_SCROLL_ACCEL_FACTOR)
                                    .log(SMOOTH_SCROLL_ACCEL_FACTOR);
                        } else {
                    ...
                    if smooth_scroll.at != 0.0 || smooth_scroll.to != 0.0 {
                        if smooth_scroll.at == smooth_scroll.to {
                            ...
                            let amt = if f32::abs(step) < 0.005 { diff } else { step };
                            ...
                            if let Some(bin_id) = &*focused {
                                for (hook_id, (hb_wk, hook, func)) in &mut *hooks {
                                    ...
                                    if hb.id() == *bin_id {
                                        match hook.ty() {
                                            BinHookTy::Press => {
                                                if let BinHookData::Press { mouse_active, .. } =
                                                    hook
                                                {
                                                    if let Some(v) = mouse_active.get_mut(&button) {
                                                        *v = false;
                                    ...
                                    if hb.id() == *bin_id {
                                        match hook.ty() {
                                            ...
                                            BinHookTy::Release => {
                                                ...
                                                if let BinHookData::Release { key_active, .. } =
                                                    hook
                                                {
                                                    if let Some(v) = key_active.get_mut(&key) {
                                                        if !*v {
