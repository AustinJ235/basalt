    ...
    pub fn new(options: Options) -> Result<Arc<Self>, String> {
        unsafe {
            ...
            if !basalt_ret.options.ignore_dpi {
                basalt_ret.interface_ref().set_scale(
                    basalt_ret.surface.window().get_hidpi_factor() as f32
                        * basalt_ret.options.scale,
                );
            } else if basalt_ret.options.scale != 1.0 {
                basalt_ret
                    .interface_ref()
                    .set_scale(basalt_ret.options.scale);
            }
            ...
            basalt_ret.input_ref().add_hook(
                ...
                Arc::new(move |_| {
                    ...
                    if basalt.options.ignore_dpi {
                        ...
                        println!(
                            "Current Scale: {:.1} %",
                            basalt.current_scale_with_dpi() * 100.0
                        );
                    }
            ...
            BasaltEvent::WindowResized => {
                if self.options.ignore_dpi {
                    if let Some((count, last, w, h)) = &mut *self.ignore_dpi_data.lock() {
                        ...
                        if *count == 1 {
                            self.surface.window().set_inner_size(
                                winit::dpi::PhysicalSize::new(*w as f64, *h as f64)
                                    .to_logical(self.surface.window().get_hidpi_factor()),
                            );
                        } else if *count == 3 && last.elapsed() < Duration::from_millis(1000) {
                            // TODO: Only if right click released
                            self.surface.window().set_inner_size(
                                winit::dpi::PhysicalSize::new(*w as f64, *h as f64)
                                    .to_logical(self.surface.window().get_hidpi_factor()),
                            );
            ...
            BasaltEvent::DPIChanged(dpi) => {
                if self.options.ignore_dpi {
                    ...
                    *self.ignore_dpi_data.lock() = Some((0, Instant::now(), ws[0], ws[1]));
                    ...
                    self.interface_ref()
                        .set_scale(dpi as f32 * *self.custom_scale.lock());
                }
            }
        }
    }
    ...
    pub fn set_scale(&self, to: f32) {
        ...
        *custom_scale = to;
        ...
        if self.options.ignore_dpi {
            ...
            self.interface_ref()
                .set_scale(*custom_scale * self.surface.window().get_hidpi_factor() as f32);
        }
    }
        ...
        'resize: loop {
            ...
            *self.window_size.lock() = [x, y];
            ...
            let (swapchain, images) = (
                &swapchain_.as_ref().unwrap().0,
                &swapchain_.as_ref().unwrap().1,
            );
            ...
            loop {
                    ...
                    if let Some(resize_to) = self.resize_to.lock().take() {
                        match resize_to {
                            ...
                            ResizeTo::Dims(w, h) => {
                                self.surface
                                    .window()
                                    .set_inner_size(if self.options.ignore_dpi {
                                        winit::dpi::PhysicalSize::new(w as f64, h as f64)
                                            .to_logical(self.surface.window().get_hidpi_factor())
                                    } else {
                ...
                let mut future: Box<GpuFuture> =
                    Box::new(previous_frame.join(acquire_future)) as Box<_>;
