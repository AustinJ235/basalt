...
pub enum InputHook {
    ...
    AnyMouseOrKeyPress { global: bool },
    /// Called on any mouse button press.
    AnyMousePress { global: bool },
    /// Called on any key press.
    AnyKeyPress { global: bool },
    ...
    AnyMouseOrKeyRelease { global: bool },
...
impl InputHook {
    pub fn into_data(&self) -> InputHookData {
        match self {
            ...
            InputHook::MouseEnter => InputHookData::MouseEnter {
                mouse_x: 0.0,
                mouse_y: 0.0,
            },

            InputHook::MouseLeave => InputHookData::MouseLeave {
                mouse_x: 0.0,
                mouse_y: 0.0,
            },

            InputHook::MouseMove => InputHookData::MouseMove {
                mouse_x: 0.0,
                mouse_y: 0.0,
                mouse_dx: 0.0,
                mouse_dy: 0.0,
            ...
            InputHook::MouseMotion => InputHookData::MouseMotion { x: 0.0, y: 0.0 },

            InputHook::MouseScroll => InputHookData::MouseScroll {
                scroll_amt: 0.0,
                mouse_x: 0.0,
                mouse_y: 0.0,
            },
            ...
            InputHook::AnyMouseOrKeyPress { global } => InputHookData::AnyMouseOrKeyPress {
                global: *global,
                either: KeyOrMouseButton::Key(Qwery::Space),
            },

            InputHook::AnyMousePress { global } => InputHookData::AnyMousePress {
                global: *global,
                button: MouseButton::Left,
            },

            InputHook::AnyKeyPress { global } => InputHookData::AnyKeyPress {
                global: *global,
                key: Qwery::Space,
            },

            InputHook::AnyMouseOrKeyRelease { global } => InputHookData::AnyMouseOrKeyRelease {
                global: *global,
                either: KeyOrMouseButton::Key(Qwery::Space),
            },

            InputHook::AnyMouseRelease { global } => InputHookData::AnyMouseRelease {
                global: *global,
                button: MouseButton::Left,
            },

            InputHook::AnyKeyRelease { global } => InputHookData::AnyKeyRelease {
                global: *global,
                key: Qwery::Space,
            },
        }
...
impl Input {
    ...
    pub fn add_hook(&self, hook: InputHook, func: InputHookFn) -> InputHookID {
        ...
        self.event_send
            .send(Event::AddHook(id, hook, func))
            .unwrap();
        id
    }
    ...
    pub(crate) fn new(basalt: Arc<Basalt>) -> Arc<Self> {
        ...
        unsafe {
            ...
            input.add_hook(
                InputHook::AnyKeyPress { global: false },
                Arc::new(move |data| {
            ...
            let interface = input.basalt.interface();
            input.add_hook(
                InputHook::AnyKeyRelease { global: false },
                Arc::new(move |data| {
            ...
            let interface = input.basalt.interface();
            input.add_hook(
                InputHook::AnyMousePress { global: false },
                Arc::new(move |data| {
                    if let InputHookData::AnyMousePress { button, .. } = data {
                        interface
                            .hook_manager
                            .send_event(ItfInputEvent::MousePress(transmute(button.clone())));
                    }
            ...
            let interface = input.basalt.interface();
            input.add_hook(
                InputHook::AnyMouseRelease { global: false },
                Arc::new(move |data| {
                    if let InputHookData::AnyMouseRelease { button, .. } = data {
                        interface
                            .hook_manager
                            .send_event(ItfInputEvent::MouseRelease(transmute(button.clone())));
                    }
                        ...
                        Event::KeyPress(k) => {
                            if window_focused {
                                for (_hook_id, (ref mut hook_data, hook_func)) in &mut hook_map {
                                    ...
                                    if let InputHookData::Character { character } = hook_data {
                                        let shift =
                                            *key_state.entry(Qwery::LShift).or_insert(false)
                            ...
                            for (_hook_id, (ref mut hook_data, hook_func)) in &mut hook_map {
                                ...
                                match hook_data.ty() {
                                    ...
                                    InputHookTy::AnyKeyPress => {
                                        if let InputHookData::AnyKeyPress { global, key } =
                                            hook_data
                                        {
                                            if (*global && !global_reject)
                                                || (!*global && window_focused)
                            ...
                            for (_hook_id, (ref mut hook_data, hook_func)) in &mut hook_map {
                                ...
                                match hook_data.ty() {
                                    ...
                                    InputHookTy::AnyKeyRelease => {
                                        if let InputHookData::AnyKeyRelease { global, key } =
                                            hook_data
                                        {
                                            if (*global && !window_focus_lost)
                                                || (!*global
                            ...
                            for (_hook_id, (ref mut hook_data, hook_func)) in &mut hook_map {
                                ...
                                match hook_data.ty() {
                                    ...
                                    InputHookTy::AnyMousePress => {
                                        if let InputHookData::AnyMousePress { global, button } =
                                            hook_data
                                        {
                                            if *global || window_focused {
                                                *button = b.clone();
                            ...
                            for (_hook_id, (ref mut hook_data, hook_func)) in &mut hook_map {
                                ...
                                match hook_data.ty() {
                                    ...
                                    InputHookTy::AnyMouseRelease => {
                                        if let InputHookData::AnyMouseRelease { global, button } =
                                            hook_data
                                        {
                                            if *global || window_focused {
                                                *button = b.clone();
                    ...
                    match hook_data.ty() {
                        ...
                        InputHookTy::Release => {
                            ...
                            if cond_change {
                                ...
                                if let InputHookData::Release { pressed, .. } = hook_data {
                                    if cond_met {
                                        if !*pressed {
                                            *pressed = true;
                                        }
                        ...
                        InputHookTy::Hold => {
                            ...
                            if cond_change {
                                ...
                                if hook_act && !cond_met {
                                    if let InputHookData::Hold { active, .. } = hook_data {
                                        *active = false;
                                        hook_act = false;
                                    }
                                }
                            ...
                            if hook_act {
                                ...
                                if call {
                                    ...
                                    if let InputHookData::Hold { is_first_call, .. } = hook_data {
                                        *is_first_call = false;
                                    }
                                }
                            }
