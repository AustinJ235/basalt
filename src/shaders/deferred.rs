...
pub mod deferred_fs {
	...
	void main() {
		...
		if(depth >= FAR_PLANE && trans_depth >= FAR_PLANE) {
			...
			float dist_from_sun = line_point_dist(zero.xyz, zero.xyz-other.sun_direction, w_pos);
		...
		for(int i = 0; i < other.samples; i++) {
			vec3 sample_pos = world_pos + (uni.samples[i] * sample_scale);
		...
		float diffuse = (1.0 - max(dot(normal, other.sun_direction), 0.0)) * 0.5 + 0.5;
