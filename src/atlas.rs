        ...
        match &mut data {
            &mut ImageData::D8(ref mut d) => {
                if d.len() > expected_len {
                    ...
                    return Err(format!(
                        "Data length doesn't match the provided dimensions."
                    ));
                }
            }
            _ => unreachable!(),
                ...
                while let Ok(cmd) = atlas.cmd_queue.pop() {
                    ...
                    match cmd {
                        Command::Upload(upreq) => {
                            ...
                            sub_img_id_count += 1;
                            let coords =
                                region.coords(atlas_image_i as u64, sub_img_id, &upreq.image.dims);
                ...
                if execute {
                    ...
                    for (i, atlas_image) in atlas_images.iter_mut().enumerate() {
                        if let Some(tmp_img) = atlas_image.complete_update() {
                            draw_map.insert(
                                (i + 1) as u64,
                                Arc::new(tmp_img) as Arc<ImageViewAccess + Send + Sync>,
                            );
                        }
                    }
    ...
    fn update(
        ...
        if found_op.is_none() || resize {
            ...
            if img_i < self.sto_imgs.len() {
                ...
                cmd_buf = cmd_buf
                    .clear_color_image(image.clone(), [0_32; 4].into())
                    .unwrap();
