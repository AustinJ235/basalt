...
impl Drop for CoordsInner {
    fn drop(&mut self) {
        ...
        atlas
            .cmd_send
            .send(Command::Dropped(*img_id, *sub_img_id))
            .unwrap();
...
enum Command {
    ...
    BatchCacheIDLookup(
        Arc<CommandResponse<Vec<Option<AtlasCoords>>>>,
        Vec<SubImageCacheID>,
    ),
...
impl Atlas {
            ...
            loop {
                ...
                for (response, cache_id, cache_ctrl, image) in upload_cmds {
                    ...
                    if coords_op.is_none() {
                        let mut atlas_image = AtlasImage::new(atlas.clone(), atlas_images.len());
    ...
    pub fn cache_coords(&self, cache_id: SubImageCacheID) -> Option<AtlasCoords> {
        ...
        self.cmd_send
            .send(Command::CacheIDLookup(response.clone(), cache_id))
            .unwrap();
    ...
    pub fn batch_cache_coords(&self, cache_ids: Vec<SubImageCacheID>) -> Vec<Option<AtlasCoords>> {
        let response = CommandResponse::new();
        self.cmd_send
            .send(Command::BatchCacheIDLookup(response.clone(), cache_ids))
            .unwrap();
        self.unparker.unpark();
        response.wait_for_response()
    }
...
impl AtlasImage {
    fn new(atlas: Arc<Atlas>, index: usize) -> Self {
        AtlasImage {
            ...
            allocator: Guillotiere::with_options(
                [512; 2].into(),
                &GuillotiereOptions {
    ...
    fn update(
        ...
        if self.views.is_empty() {
            ...
            if view_updates.is_empty() {
                if pending_update
                    && (self.active.is_none() || self.views[*self.active.as_ref().unwrap()].stale)
                {
    ...
    fn try_allocate(
        ...
        let alloc_size = [
            ...
            image.dims.h as i32 + ALLOC_PAD_2X,
        ]
        .into();
