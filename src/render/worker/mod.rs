...
impl Drop for Worker {
    fn drop(&mut self) {
        ...
        println!("[Worker.drop][3/5]: vertex flight wait idle");

        resources.flight(self.vertex_flt_id).unwrap().wait_idle().unwrap();
        
        println!("[Worker.drop][4/5]: image flight wait idle");

        resources.flight(self.image_flt_id).unwrap().wait_idle().unwrap();
